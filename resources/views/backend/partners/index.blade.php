@extends('layouts.app')
@section('title', 'Partnership List')
@section('content')
                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        ...
                        <div class="card-header">
                            <h4>Partner Breadcrumb Details</h4>
                        </div>
                        <div class="card-body">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr>
                                        ...
                                        <td>{{ $partnerHeading->small_heading ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <th class="custom-th">Heading</th>
                                        <td>{{ $partnerHeading->heading ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <th class="custom-th">Heading Description</th>
                                        <td>{{ $partnerHeading->heading_description ?? '' }}</td>
                                    </tr>
                            ...
                            <a href="{{ route('backend.partners_heading.edit', $partnerHeading?->id) }}" class="btn btn-primary">Update</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h4>Partner List</h4>
                            <a class="btn btn-primary btn-sm" href="{{ route('backend.partners.create') }}" style="padding: 5px 25px;">Add</a>
                        </div>
                        ...
                        <table class="table table-bordered">
                                    ...
                                    @if($partners->count() > 0)
                                        @foreach($partners as $key => $partner)
                                            <tr>
                                                ...
                                                <td>
                                                     @if ($partner->image)
                                        <img src="{{ asset('storage/' . $partner->image) }}" class="mt-2" width="50">
                                    @endif
                                </td>
                                                <td>
                                                    @if ($partner->is_publish == 'Publish')
                                                ...
                                                <td align="center">
                                                <a href="{{ route('backend.partners.edit', $partner->id) }}" class="btn btn-primary">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                    <form action="{{ route('backend.partners.destroy', $partner->id) }}" method="POST" style="display:inline;" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
                            ...
                            <nav aria-label="Portfolio pagination">
                                <ul class="pagination justify-content-end">
                                    ...
                                    @else
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $partners->previousPageUrl() }}">Previous</a>
                                        </li>
                                    @endif
