...
<div class="main-content">
                            ...
                            <table class="table table-bordered">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{{ $careerHeading->small_heading ?? '' }}</td>
                                    </tr>

                                    <tr>
                                        <th class="custom-th">Heading</th>
                                        <td>{{ $careerHeading->heading ?? '' }}</td>
                                    </tr>

                                    <tr>
                                        <th class="custom-th">Heading Description</th>
                                        <td>{!! $careerHeading->heading_description ?? '' !!}</td>
                                    </tr>

                                    <tr>
                                        <th class="custom-th">SEO Title</th>
                                        <td>{{ $careerHeading->seo_title ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <th class="custom-th">SEO Description</th>
                                        <td>{{ $careerHeading->seo_description ?? '' }}</td>
                                    </tr>
                                    <tr>
                                    <tr>
    ...
    <td>
        @php
            $decodedKeywords = $careerHeading && $careerHeading->seo_keywords
                ? json_decode($careerHeading->seo_keywords, true)
                : [];
        @endphp

        @if(is_array($decodedKeywords) && count($decodedKeywords))
...
<tr>
    ...
    <td>
        @if($careerHeading && $careerHeading->seo_image)
            <img src="{{ asset('storage/' . $careerHeading->seo_image) }}" alt="SEO Image" width="300">
        @else
        ...
        @endif
                            ...
                            <div class="card-footer text-right">
                                <a href="{{ route('backend.career_heading.edit') }}" class="btn btn-primary">Update</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h4>Career List</h4>
                            <a class="btn btn-primary btn-sm" href="{{ route('backend.careers.create') }}" style="padding: 5px 25px;">Add New Services</a>
                        </div>
                        ...
                        <div class="card-body">
                                    ...
                                    @if($careers->count() > 0)
                                        @foreach($careers as $key => $career)
                                            <tr>
                                                ...
                                                <td align="center">
                                                    <a href="{{ route('backend.careers.edit', $career->id) }}" class="btn btn-primary">
                                                    <i class="fas fa-edit"></i> Edit</a>

                                                    <form action="{{ route('backend.careers.destroy', $career->id) }}" method="POST" style="display:inline;" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
                            ...
                            <nav aria-label="User pagination">
                                <ul class="pagination justify-content-end">
                                    ...
                                    @else
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $careers->previousPageUrl() }}">Previous</a>
                                        </li>
                                    @endif

                                    @foreach (range(1, $careers->lastPage()) as $page)
                                        <li class="page-item {{ $page == $careers->currentPage() ? 'active' : '' }}">
                                            <a class="page-link" href="{{ $careers->url($page) }}">{{ $page }}</a>
                                        </li>
                                    @endforeach

                                    @if ($careers->hasMorePages())
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $careers->nextPageUrl() }}">Next</a>
                                        </li>
                                    @else
                                        <li class="page-item disabled">
