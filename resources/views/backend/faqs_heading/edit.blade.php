                ...
                @include('_message')
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4>Faqs Heading Details</h4>
                            <a href="{{ route('backend.faqs.index') }}" class="btn btn-primary btn-sm ml-auto">Back to the List</a>
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST" action="{{ route('backend.faqs_heading.update', $faqsHeading->id) }}">
                                        @csrf
                                        @method('PUT') <!-- This converts POST to PUT -->

                                <div class="form-group">
                                    <label>Small heading</label>
                                    <input type="text" name="small_heading" class="form-control" value="{{ old('small_heading', $faqsHeading->small_heading ?? '') }}">
                                </div>
                                <div class="form-group">
                                    <label>Heading</label>
                                    <input type="text" name="heading" class="form-control" value="{{ old('heading', $faqsHeading->heading ?? '') }}" required>
                                </div>

                                <div class="form-group">
                                    <label>Heading Description</label>
                                    <textarea name="heading_description" class="form-control">{{ old('heading_description', $faqsHeading->heading_description ?? '') }}</textarea>
                                </div>
